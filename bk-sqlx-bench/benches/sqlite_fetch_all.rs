use bk_sqlx::{Connection, Executor};
...
fn main() -> bk_sqlx::Result<()> {
    bk_sqlx::__rt::block_on(async {
        let mut conn = bk_sqlx::SqliteConnection::connect("sqlite://test.db?mode=rwc").await?;
        ...
        for entry in 0i32..100000 {
            bk_sqlx::query("INSERT INTO test (id) VALUES ($1)")
                .bind(entry)
                .execute(&mut tx)
                .await?;
        }
        ...
        for _ in 0..10i8 {
            ...
            println!(
                "total: {}",
                bk_sqlx::query!("SELECT id from test")
                    .fetch_all(&mut conn)
                    .await?
                    .len()
            );
