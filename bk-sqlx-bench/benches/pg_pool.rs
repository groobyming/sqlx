...
fn do_bench_acquire(b: &mut Bencher, concurrent: u32, fair: bool) {
    let pool = bk_sqlx::__rt::block_on(
        PgPoolOptions::new()
    ...
    for _ in 0..concurrent {
        let pool = pool.clone();
        bk_sqlx::__rt::enter_runtime(|| {
            bk_sqlx::__rt::spawn(async move {
                while !pool.is_closed() {
                    let conn = match pool.acquire().await {
                        Ok(conn) => conn,
                        Err(bk_sqlx::Error::PoolClosed) => break,
                        Err(e) => panic!("failed to acquire concurrent connection: {e}"),
                    };

                    // pretend we're using the connection
                    bk_sqlx::__rt::sleep(Duration::from_micros(500)).await;
    ...
    bk_sqlx::__rt::block_on(pool.close());
}
