use bk_sqlx::{error::ErrorKind, sqlite::Sqlite, Connection, Executor};
...
async fn it_fails_with_unique_violation() -> anyhow::Result<()> {
    ...
    let res: Result<_, bk_sqlx::Error> = bk_sqlx::query("INSERT INTO tweet VALUES (1, 'Foo', true, 1);")
        .execute(&mut *tx)
        .await;
...
async fn it_fails_with_foreign_key_violation() -> anyhow::Result<()> {
    ...
    let res: Result<_, bk_sqlx::Error> =
        bk_sqlx::query("INSERT INTO tweet_reply (id, tweet_id, text) VALUES (2, 2, 'Reply!');")
            .execute(&mut *tx)
            .await;
    let err = res.unwrap_err();
