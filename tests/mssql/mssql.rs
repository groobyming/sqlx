...
use bk_sqlx::mssql::{Mssql, MssqlPoolOptions};
use bk_sqlx::{Column, Connection, Executor, MssqlConnection, Row, Statement, TypeInfo};
use bk_sqlx_core::mssql::MssqlRow;
...
async fn it_maths() -> anyhow::Result<()> {
    ...
    let value = bk_sqlx::query("SELECT 1 + @p1")
        .bind(5_i32)
...
CREATE TABLE #users (id INTEGER PRIMARY KEY);
    ...
    for index in 1..=10_i32 {
        let done = bk_sqlx::query("INSERT INTO #users (id) VALUES (@p1)")
            .bind(index * 2)
            .execute(&mut conn)
            .await?;
...
async fn it_selects_null() -> anyhow::Result<()> {
    ...
    let (_, val): (i32, Option<i32>) = bk_sqlx::query_as("SELECT 5, NULL")
        .fetch_one(&mut conn)
        .await?;
...
async fn it_binds_empty_string_and_null() -> anyhow::Result<()> {
    ...
    let (val, val2): (String, Option<String>) = bk_sqlx::query_as("SELECT @p1, @p2")
        .bind("")
        .bind(None::<String>)
        .fetch_one(&mut conn)
        .await?;
...
async fn it_can_work_with_transactions() -> anyhow::Result<()> {
    ...
    conn.execute("IF OBJECT_ID('_bk_sqlx_users_1922', 'U') IS NULL CREATE TABLE _bk_sqlx_users_1922 (id INTEGER PRIMARY KEY)")
        .await?;

    conn.execute("DELETE FROM _bk_sqlx_users_1922").await?;
    ...
    let (count,): (i32,) = bk_sqlx::query_as("SELECT COUNT(*) FROM _bk_sqlx_users_1922")
        .fetch_one(&mut conn)
        .await?;
    ...
    bk_sqlx::query("INSERT INTO _bk_sqlx_users_1922 (id) VALUES (@p1)")
        .bind(10_i32)
        .execute(&mut tx)
        .await?;
...
async fn it_can_work_with_nested_transactions() -> anyhow::Result<()> {
    ...
    conn.execute("IF OBJECT_ID('_bk_sqlx_users_2523', 'U') IS NULL CREATE TABLE _bk_sqlx_users_2523 (id INTEGER PRIMARY KEY)")
        .await?;

    conn.execute("DELETE FROM _bk_sqlx_users_2523").await?;

    // begin
    let mut tx = conn.begin().await?;

    // insert a user
    bk_sqlx::query("INSERT INTO _bk_sqlx_users_2523 (id) VALUES (@p1)")
        .bind(50_i32)
        .execute(&mut tx)
        .await?;
    ...
    // did we really?
    let (count,): (i32,) = bk_sqlx::query_as("SELECT COUNT(*) FROM _bk_sqlx_users_2523")
        .fetch_one(&mut tx)
        .await?;
...
async fn it_can_prepare_then_execute() -> anyhow::Result<()> {
    ...
    let tweet_id: i64 = bk_sqlx::query_scalar(
