...
pub enum Command {
    ...
    ///
    /// Saves metadata for all invocations of `query!` and related macros to a `.bk_sqlx` directory
    ...
    Prepare {
        ...
        /// Generate a single workspace-level `.bk_sqlx` folder.
        ///
        /// This option is intended for workspaces where multiple crates use SQLx. If there is only
        /// one, it is better to run `cargo bk_sqlx prepare` without this option inside that crate.
        #[clap(long)]
        workspace: bool,
...
pub struct ConnectOpts {
    ...
    ///
    /// WAL mode is enabled by default for SQLite databases created by `bk_sqlx-cli`.
    ///
