...
impl PrepareCtx {
    ...
    fn prepare_dir(&self) -> anyhow::Result<PathBuf> {
        if self.workspace {
            Ok(self.metadata.workspace_root().join(".bk_sqlx"))
        } else {
            Ok(manifest_dir(&self.cargo)?.join(".bk_sqlx"))
        }
    }
}
...
pub async fn run(
    ...
    let cargo = env::var_os("CARGO")
        .context("failed to get value of `CARGO`; `prepare` subcommand may only be invoked as `cargo bk_sqlx prepare`")?;
...
async fn prepare(ctx: &PrepareCtx) -> anyhow::Result<()> {
    ...
    if ctx.workspace {
        println!(
            "query data written to .bk_sqlx in the workspace root; \
             please check this into version control"
        );
    } else {
        println!(
            "query data written to .bk_sqlx in the current directory; \
             please check this into version control"
        );
    }
    Ok(())
...
async fn prepare_check(ctx: &PrepareCtx) -> anyhow::Result<()> {
    ...
    let prepare_dir = ctx.prepare_dir()?;
    let cache_dir = ctx.metadata.target_directory().join("bk_sqlx-prepare-check");
    run_prepare_step(ctx, &cache_dir)?;

    // Compare .bk_sqlx to cache.
    ...
    // Error: files in cache but not .bk_sqlx.
    if cache_filenames
        .difference(&prepare_filenames)
        .next()
        .is_some()
    {
        bail!("prepare check failed: .bk_sqlx is missing one or more queries; you should re-run bk_sqlx prepare");
    }
    // Warn: files in .bk_sqlx but not cache.
    ...
    {
        println!(
            "{} potentially unused queries found in .bk_sqlx; you may want to re-run bk_sqlx prepare",
            style("warning:").yellow()
        );
    }

    // Compare file contents as JSON to ignore superficial differences.
    // Everything in cache checked to be in .bk_sqlx already.
    for filename in cache_filenames {
        ...
        if prepare_json != cache_json {
            bail!("prepare check failed: one or more query files differ ({}); you should re-run bk_sqlx prepare", filename);
        }
    }
...
fn run_prepare_step(ctx: &PrepareCtx, cache_dir: &Path) -> anyhow::Result<()> {
    ...
    let tmp_dir = ctx.metadata.target_directory().join("bk_sqlx-tmp");
    fs::create_dir_all(&tmp_dir).context(format!(
        "Failed to create temporary query cache directory: {:?}",
        cache_dir
    ))?;

    // Only delete bk_sqlx-*.json files to avoid accidentally deleting any user data.
    ...
    // Try only triggering a recompile on crates that use `bk_sqlx-macros` falling back to a full
    // clean on error
...
/// Sets up recompiling only crates that depend on `bk_sqlx-macros`
///
/// This gets a listing of all crates that depend on `bk_sqlx-macros` (direct and transitive). The
...
fn minimal_project_recompile_action(metadata: &Metadata) -> ProjectRecompileAction {
    // Get all the packages that depend on `bk_sqlx-macros`
    let mut bk_sqlx_macros_dependents = BTreeSet::new();
    let bk_sqlx_macros_ids: BTreeSet<_> = metadata
        .entries()
        ...
        .filter(|(_, package)| package.name() == "bk_sqlx-macros")
        .map(|(id, _)| id)
        .collect();
    for bk_sqlx_macros_id in bk_sqlx_macros_ids {
        bk_sqlx_macros_dependents.extend(metadata.all_dependents_of(bk_sqlx_macros_id));
    }

    // Figure out which `bk_sqlx-macros` dependents are in the workspace vs out
    let mut in_workspace_dependents = Vec::new();
    let mut out_of_workspace_dependents = Vec::new();
    for dependent in bk_sqlx_macros_dependents {
...
mod tests {
    ...
    fn minimal_project_recompile_action_works() -> anyhow::Result<()> {
        ...
        assert_eq!(
            action,
            ProjectRecompileAction {
                clean_packages: vec!["bk_sqlx".into()],
                touch_paths: vec![
