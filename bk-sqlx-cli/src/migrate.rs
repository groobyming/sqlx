...
use chrono::Utc;
use console::style;
use bk_sqlx::migrate::{AppliedMigration, Migrate, MigrateError, MigrationType, Migrator};
...
bk_sqlx::migrate!({quoted_source}).run(<&your_pool OR &mut your_connection>).await?;

Note that the compiler won't pick up new migrations if no Rust source files have changed.
You can create a Cargo build script to work around this with `bk_sqlx migrate build-script`.

See: https://docs.rs/bk_sqlx/{version}/bk_sqlx/macro.migrate.html
"#,
        );
    }
...
pub fn build_script(migration_source: &str, force: bool) -> anyhow::Result<()> {
    ...
    let contents = format!(
        r#"// generated by `bk_sqlx migrate build-script`
fn main() {{
