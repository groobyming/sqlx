...
pub fn parse_container_attributes(input: &[Attribute]) -> syn::Result<SqlxContainerAttributes> {
    ...
    for attr in input
        .iter()
        .filter(|a| a.path.is_ident("bk_sqlx") || a.path.is_ident("repr"))
    {
        let meta = attr
            .parse_meta()
            .map_err(|e| syn::Error::new_spanned(attr, e))?;
        match meta {
            Meta::List(list) if list.path.is_ident("bk_sqlx") => {
...
pub fn parse_child_attributes(input: &[Attribute]) -> syn::Result<SqlxChildAttributes> {
    ...
    for attr in input.iter().filter(|a| a.path.is_ident("bk_sqlx")) {
        let meta = attr
            .parse_meta()
            .map_err(|e| syn::Error::new_spanned(attr, e))?;
...
pub fn check_enum_attributes(input: &DeriveInput) -> syn::Result<SqlxContainerAttributes> {
    ...
    assert_attribute!(
        ...
        "unexpected #[bk_sqlx(transparent)]",
        input
    );

    assert_attribute!(
        !attributes.no_pg_array,
        "unused #[bk_sqlx(no_pg_array)]; derive does not emit `PgHasArrayType` impls for enums",
        input
    );
...
pub fn check_struct_attributes<'a>(
    ...
    assert_attribute!(
        !attributes.no_pg_array,
        "unused #[bk_sqlx(no_pg_array)]; derive does not emit `PgHasArrayType` impls for custom structs",
        input
    );
