#[cfg(any(bk_sqlx_macros_unstable, procmacro2_semver_exempt))]
...
use bk_sqlx_core::migrate::{Migration, MigrationType};
...
impl ToTokens for QuoteMigrationType {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        let ts = match self.0 {
            MigrationType::Simple => quote! { ::bk_sqlx::migrate::MigrationType::Simple },
            MigrationType::ReversibleUp => quote! { ::bk_sqlx::migrate::MigrationType::ReversibleUp },
            MigrationType::ReversibleDown => {
                quote! { ::bk_sqlx::migrate::MigrationType::ReversibleDown }
            }
        };
...
pub(crate) fn expand_migrator(path: &Path) -> crate::Result<TokenStream> {
    ...
    let migrations = bk_sqlx_core::migrate::resolve_blocking(path)?
        .into_iter()
    ...
    Ok(quote! {
        ::bk_sqlx::migrate::Migrator {
            migrations: ::std::borrow::Cow::Borrowed(&[
                    #(#migrations),*
            ]),
            ..::bk_sqlx::migrate::Migrator::DEFAULT
        }
    })
}
