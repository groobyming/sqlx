...
//! The only crate that follows Semantic Versioning in the project is the `bk_sqlx` crate itself.
//! If you are building a custom SQLx driver, you should pin an exact version of this and
//! `bk_sqlx-core` to avoid breakages:
//!
//! ```toml
//! bk_sqlx-core = "=0.6.2"
//! bk_sqlx-macros-core = "=0.6.2"
//! ```
//!
//! And then make releases in lockstep with `bk_sqlx-core`. We recommend all driver crates, in-tree
//! or otherwise, use the same version numbers as `bk_sqlx-core` to avoid confusion.

#![cfg_attr(
    any(bk_sqlx_macros_unstable, procmacro2_semver_exempt),
    feature(track_path)
)]
...
pub const FOSS_DRIVERS: &[QueryDriver] = &[
    #[cfg(feature = "mysql")]
    QueryDriver::new::<bk_sqlx_mysql::MySql>(),
    #[cfg(feature = "postgres")]
    QueryDriver::new::<bk_sqlx_postgres::Postgres>(),
    #[cfg(feature = "sqlite")]
    QueryDriver::new::<bk_sqlx_sqlite::Sqlite>(),
];
