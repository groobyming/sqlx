...
fn expand_simple(input: syn::ItemFn) -> TokenStream {
    ...
    quote! {
        ...
        #(#attrs)*
        fn #name() #ret {
            ::bk_sqlx::test_block_on(async { #body })
        }
    }
}
...
fn expand_advanced(args: syn::AttributeArgs, input: syn::ItemFn) -> crate::Result<TokenStream> {
    ...
    Ok(quote! {
        ...
        fn #name() #ret {
            ...
            let mut args = ::bk_sqlx::testing::TestArgs::new(concat!(module_path!(), "::", stringify!(#name)));
            ...
            ::bk_sqlx::testing::TestFn::run_test(f, args)
        }
    })
}
