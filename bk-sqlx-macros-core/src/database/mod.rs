...
pub trait DatabaseExt: Database {
    ...
    fn describe_blocking(query: &str, database_url: &str) -> bk_sqlx_core::Result<Describe<Self>>;
}
...
impl<DB: DatabaseExt> CachingDescribeBlocking<DB> {
    ...
    pub fn describe(&self, query: &str, database_url: &str) -> bk_sqlx_core::Result<Describe<DB>>
    where
        ...
        impl $crate::database::DatabaseExt for $database {
            ...
            fn param_type_for_id(info: &Self::TypeInfo) -> Option<&'static str> {
                match () {
                    $(
                        $(#[$meta])?
                        _ if <$ty as bk_sqlx_core::types::Type<$database>>::type_info() == *info => Some(input_ty!($ty $(, $input)?)),
                    )*
                    $(
                        $(#[$meta])?
                        _ if <$ty as bk_sqlx_core::types::Type<$database>>::compatible(info) => Some(input_ty!($ty $(, $input)?)),
                    )*
                    _ => None
                }
            }
            ...
            fn return_type_for_id(info: &Self::TypeInfo) -> Option<&'static str> {
                match () {
                    $(
                        $(#[$meta])?
                        _ if <$ty as bk_sqlx_core::types::Type<$database>>::type_info() == *info => return Some(stringify!($ty)),
                    )*
                    $(
                        $(#[$meta])?
                        _ if <$ty as bk_sqlx_core::types::Type<$database>>::compatible(info) => return Some(stringify!($ty)),
                    )*
                    _ => None
                }
            }
...
macro_rules! impl_describe_blocking {
    ($database:path $(,)?) => {
        fn describe_blocking(
            query: &str,
            database_url: &str,
        ) -> bk_sqlx_core::Result<bk_sqlx_core::describe::Describe<Self>> {
...
mod fake_bk_sqlx {
    pub use bk_sqlx_core::*;

    #[cfg(feature = "mysql")]
    pub use bk_sqlx_mysql as mysql;

    #[cfg(feature = "postgres")]
    pub use bk_sqlx_postgres as postgres;

    #[cfg(feature = "sqlite")]
    pub use bk_sqlx_sqlite as sqlite;
}
