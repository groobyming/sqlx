...
use bk_sqlx_core::{column::Column, describe::Describe, type_info::TypeInfo};
...
pub fn expand_input<'a>(
    ...
    let data_source = match &*METADATA {
        ...
        Metadata { offline, .. } => {
            ...
            // Check SQLX_OFFLINE_DIR, then local .bk_sqlx, then workspace .bk_sqlx.
            let dirs = [
                || env("SQLX_OFFLINE_DIR").ok().map(PathBuf::from),
                || Some(METADATA.manifest_dir.join(".bk_sqlx")),
                || Some(METADATA.workspace_root().join(".bk_sqlx")),
            ];
            let Some(data_file_path) = dirs
                ...
                return Err(
                    if *offline {
                        "`SQLX_OFFLINE=true` but there is no cached data for this query, run `cargo bk_sqlx prepare` to update the query cache or unset `SQLX_OFFLINE`"
                    } else {
                        "set `DATABASE_URL` to use query macros online, or run `cargo bk_sqlx prepare` to update the query cache"
                    }.into()
                );
            };
    ...
    Describe<DB>: DescribeExt,
{
        ...
        .all(|it| it.type_info().is_void())
    {
        ...
        quote! {
            ::bk_sqlx::query_with::<#db_path, _>(#sql, #query_args)
        }
    } else {
    ...
    if !offline {
        // Only save query metadata if SQLX_OFFLINE_DIR is set manually or by `cargo bk_sqlx prepare`.
            ...
            match fs::metadata(&path) {
                Err(e) => {
                    if e.kind() != io::ErrorKind::NotFound {
                        // Can't obtain information about .bk_sqlx
                        return Err(format!("{e}: {dir}").into());
                    }
                    // .bk_sqlx doesn't exist.
                    return Err(format!("bk_sqlx offline path does not exist: {dir}").into());
                }
                Ok(meta) => {
                    if !meta.is_dir() {
                        return Err(format!(
                            "bk_sqlx offline path exists, but is not a directory: {dir}"
                        )
                        .into());
                    }

                    // .bk_sqlx exists and is a directory, store data.
                    data.save_in(path)?;
                }
            }
        }
