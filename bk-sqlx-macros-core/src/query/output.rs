...
use bk_sqlx_core::column::Column;
...
fn column_to_rust<DB: DatabaseExt>(describe: &Describe<DB>, i: usize) -> crate::Result<RustColumn> {
    ...
    Ok(RustColumn {
        ...
        // https://github.com/launchbadge/bk_sqlx/issues/1322
        var_name: quote::format_ident!("bk_sqlx_query_as_{}", decl.ident),
        ident: decl.ident,
        type_,
    })
}
...
pub fn quote_query_as<DB: DatabaseExt>(
    ...
    quote! {
        ::bk_sqlx::query_with::<#db_path, _>(#sql, #bind_args).try_map(|row: #row_path| {
            use ::bk_sqlx::Row as _;
...
pub fn quote_query_scalar<DB: DatabaseExt>(
    ...
    Ok(quote! {
        ::bk_sqlx::query_scalar_with::<#db, #ty, _>(#query, #bind_args)
    })
}
        ...
        || {
            let message =
                if let Some(feature_gate) = <DB as DatabaseExt>::get_feature_gate(&type_info) {
                    format!(
                        "optional bk_sqlx feature `{feat}` required for type {ty} of {col}",
                        ty = &type_info,
                        feat = feature_gate,
                        col = DisplayColumn {
                            idx: i,
