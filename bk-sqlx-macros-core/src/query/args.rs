...
pub fn quote_args<DB: DatabaseExt>(
    ...
    if input.arg_exprs.is_empty() {
        return Ok(quote! {
            let query_args = <#db_path as ::bk_sqlx::database::HasArguments>::Arguments::default();
        });
    }
            ...
            params
                ...
                .map(|(i, (param_ty, (name, expr)))| -> crate::Result<_> {
                            ...
                            DB::param_type_for_id(&param_ty)
                                .ok_or_else(|| {
                                    if let Some(feature_gate) = <DB as DatabaseExt>::get_feature_gate(&param_ty) {
                                        format!(
                                            "optional bk_sqlx feature `{}` required for type {} of param #{}",
                                            feature_gate,
                                            param_ty,
                                            i + 1,
                                        )
                    ...
                    Ok(quote_spanned!(expr.span() =>
                        ...
                        if false {
                            use ::bk_sqlx::ty_match::{WrapSameExt as _, MatchBorrowExt as _};

                            // evaluate the expression only once in case it contains moves
                            let expr = ::bk_sqlx::ty_match::dupe_value(#name);

                            // if `expr` is `Option<T>`, get `Option<$ty>`, otherwise `$ty`
                            let ty_check = ::bk_sqlx::ty_match::WrapSame::<#param_ty, _>::new(&expr).wrap_same();

                            // if `expr` is `&str`, convert `String` to `&str`
                            let (mut _ty_check, match_borrow) = ::bk_sqlx::ty_match::MatchBorrow::new(ty_check, &expr);
    ...
    Ok(quote! {
        ...
        let mut query_args = <#db_path as ::bk_sqlx::database::HasArguments>::Arguments::default();
        query_args.reserve(
            #args_count,
            0 #(+ ::bk_sqlx::encode::Encode::<#db_path>::size_hint(#arg_name))*
        );
