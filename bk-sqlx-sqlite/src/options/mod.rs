...
///
/// ```rust,no_run
/// # async fn example() -> bk_sqlx::Result<()> {
/// use bk_sqlx::ConnectOptions;
/// use bk_sqlx::sqlite::{SqliteConnectOptions, SqliteJournalMode};
/// use std::str::FromStr;
///
...
impl SqliteConnectOptions {
    ...
    pub fn new() -> Self {
        ...
        // https://github.com/launchbadge/bk_sqlx/pull/1930#issuecomment-1168165414
        ...
        Self {
            ...
            serialized: false,
            thread_name: Arc::new(DebugFn(|id| format!("bk_sqlx-sqlite-worker-{id}"))),
    ...
    /// databases.
    ///
    /// For consistency, any commands in `bk_sqlx-cli` which create a SQLite database will create it
    /// in WAL mode.
    ...
    /// [open an issue](https://github.com/launchbadge/bk_sqlx/issues/new/choose) as this may indicate
    ...
    /// # use std::str::FromStr;
    /// # use bk_sqlx_sqlite::SqliteConnectOptions;
    ...
    /// ```
    /// # use std::str::FromStr;
    /// # use bk_sqlx::{ConnectOptions, Connection, Row};
    /// # use bk_sqlx_sqlite::SqliteConnectOptions;
    /// # async fn run() -> bk_sqlx::Result<()> {
    /// let mut sqlite = SqliteConnectOptions::from_str("sqlite://:memory:")?
    ///     .with_regexp()
    ///     .connect()
    ///     .await?;
    /// let tables = bk_sqlx::query("SELECT name FROM sqlite_schema WHERE name REGEXP 'foo(\\d+)bar'")
    ...
    /// ```
    ///
    /// This uses the [`regex`] crate, and is only enabled when you enable the `regex` feature is enabled on bk_sqlx
    #[cfg(feature = "regexp")]
