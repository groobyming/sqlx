use bk_sqlx_core::{connection::LogSettings, logger};
use std::collections::HashSet;
use std::fmt::Debug;
use std::hash::Hash;

pub(crate) use bk_sqlx_core::logger::*;
...
impl<'q, O: Debug + Hash + Eq, R: Debug, P: Debug> QueryPlanLogger<'q, O, R, P> {
    ...
    pub fn log_enabled(&self) -> bool {
            ...
            logger::private_level_filter_to_levels(self.settings.statements_level)
        {
            log::log_enabled!(log_level)
                || bk_sqlx_core::private_tracing_dynamic_enabled!(tracing_level)
        } else {
            false
        }
    }
    ...
    pub fn finish(&self) {
        ...
        if let Some((tracing_level, log_level)) = logger::private_level_filter_to_levels(lvl) {
            let log_is_enabled = log::log_enabled!(target: "bk_sqlx::explain", log_level)
                || private_tracing_dynamic_enabled!(target: "bk_sqlx::explain", tracing_level);
            if log_is_enabled {
                ...
                bk_sqlx_core::private_tracing_dynamic_event!(
                    target: "bk_sqlx::explain",
                    tracing_level,
                    message,
                );
            }
