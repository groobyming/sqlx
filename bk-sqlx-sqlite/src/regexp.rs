...
//! - bk_sqlx has a way to safely get a sqlite3 pointer:
//!   - <https://docs.rs/bk_sqlx/0.6.2/bk_sqlx/sqlite/struct.SqliteConnection.html#method.lock_handle>
//!   - <https://docs.rs/bk_sqlx/0.6.2/bk_sqlx/sqlite/struct.LockedSqliteHandle.html#method.as_raw_handle>
...
mod tests {
    ...
    async fn test_db() -> crate::SqliteConnection {
        let mut conn = crate::SqliteConnectOptions::from_str("sqlite://:memory:")
            ...
            .await
            .unwrap();
        bk_sqlx::query("CREATE TABLE test (col TEXT NOT NULL)")
            .execute(&mut conn)
            .await
            .unwrap();
        for i in 0..10 {
            bk_sqlx::query("INSERT INTO test VALUES (?)")
                .bind(format!("value {i}"))
                .execute(&mut conn)
                .await
                .unwrap();
    ...
    async fn test_regexp_does_not_fail() {
        ...
        let result = bk_sqlx::query("SELECT col FROM test WHERE col REGEXP 'foo.*bar'")
            .fetch_all(&mut conn)
            .await
    ...
    async fn test_regexp_filters_correctly() {
        ...
        let result = bk_sqlx::query("SELECT col FROM test WHERE col REGEXP '.*2'")
            .fetch_all(&mut conn)
            .await
            .expect("Could not execute query");
        ...
        let result = bk_sqlx::query("SELECT col FROM test WHERE col REGEXP '^3'")
            .fetch_all(&mut conn)
            .await
            .expect("Could not execute query");
        assert!(result.is_empty());
    ...
    async fn test_invalid_regexp_should_fail() {
        let mut conn = test_db().await;
        let result = bk_sqlx::query("SELECT col from test WHERE col REGEXP '(?:?)'")
            .execute(&mut conn)
            .await;
        assert!(matches!(result, Err(bk_sqlx::Error::Database(_))));
    }
}
