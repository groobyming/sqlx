...
WHERE rngtypid = $1
    ...
    async fn nullables_from_explain(
        ...
        let mut explain = format!(
            "EXPLAIN (VERBOSE, FORMAT JSON) EXECUTE bk_sqlx_s_{}",
            stmt_id.0
        );
        let mut comma = false;
...
enum Explain {
    ...
    // https://github.com/launchbadge/bk_sqlx/issues/1449
    //
    ...
    // https://github.com/launchbadge/bk_sqlx/issues/2587
    // https://github.com/launchbadge/bk_sqlx/issues/2622
    Plan {
        #[serde(rename = "Plan")]
        plan: Plan,
    },
