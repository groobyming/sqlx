...
use bk_sqlx_core::connection::Connection;
use bk_sqlx_core::database::Database;
use bk_sqlx_core::describe::Describe;
use bk_sqlx_core::executor::Executor;
use bk_sqlx_core::ext::ustr::UStr;
use bk_sqlx_core::transaction::TransactionManager;

bk_sqlx_core::declare_driver_with_optional_migrate!(DRIVER = Postgres);
...
impl AnyConnectionBackend for PgConnection {
    ...
    fn close(self: Box<Self>) -> BoxFuture<'static, bk_sqlx_core::Result<()>> {
        Connection::close(*self)
    }

    fn close_hard(self: Box<Self>) -> BoxFuture<'static, bk_sqlx_core::Result<()>> {
        Connection::close_hard(*self)
    }

    fn ping(&mut self) -> BoxFuture<'_, bk_sqlx_core::Result<()>> {
        Connection::ping(self)
    }

    fn begin(&mut self) -> BoxFuture<'_, bk_sqlx_core::Result<()>> {
        PgTransactionManager::begin(self)
    }

    fn commit(&mut self) -> BoxFuture<'_, bk_sqlx_core::Result<()>> {
        PgTransactionManager::commit(self)
    }

    fn rollback(&mut self) -> BoxFuture<'_, bk_sqlx_core::Result<()>> {
    ...
    fn flush(&mut self) -> BoxFuture<'_, bk_sqlx_core::Result<()>> {
    ...
    fn as_migrate(
        &mut self,
    ) -> bk_sqlx_core::Result<&mut (dyn bk_sqlx_core::migrate::Migrate + Send + 'static)> {
        Ok(self)
    }
    ...
    fn fetch_many<'q>(
    ...
    ) -> BoxStream<'q, bk_sqlx_core::Result<Either<AnyQueryResult, AnyRow>>> {
        ...
        Box::pin(
            self.run(query, args, 0, persistent, None)
                .try_flatten_stream()
                .map(
                    move |res: bk_sqlx_core::Result<Either<PgQueryResult, PgRow>>| match res? {
    ...
    fn fetch_optional<'q>(
        ...
        query: &'q str,
        arguments: Option<AnyArguments<'q>>,
    ) -> BoxFuture<'q, bk_sqlx_core::Result<Option<AnyRow>>> {
        let persistent = arguments.is_some();
        let args = arguments.as_ref().map(AnyArguments::convert_to);
    ...
    ) -> BoxFuture<'c, bk_sqlx_core::Result<AnyStatement<'q>>> {
        Box::pin(async move {
    ...
    fn describe<'q>(&'q mut self, sql: &'q str) -> BoxFuture<'q, bk_sqlx_core::Result<Describe<Any>>> {
        Box::pin(async move {
                    ...
                    parameters
                        ...
                        .map(|(i, type_info)| {
                            AnyTypeInfo::try_from(type_info).map_err(|_| {
                                bk_sqlx_core::Error::AnyDriverError(
                                    format!(
    ...
    fn try_from(pg_type: &'a PgTypeInfo) -> Result<Self, Self::Error> {
        Ok(AnyTypeInfo {
            kind: match &pg_type.0 {
                ...
                _ => {
                    return Err(bk_sqlx_core::Error::AnyDriverError(
...
impl<'a> TryFrom<&'a PgColumn> for AnyColumn {
    ...
    fn try_from(col: &'a PgColumn) -> Result<Self, Self::Error> {
        let type_info =
            AnyTypeInfo::try_from(&col.type_info).map_err(|e| bk_sqlx_core::Error::ColumnDecode {
