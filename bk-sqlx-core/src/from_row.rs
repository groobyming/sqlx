...
///
/// ```rust, ignore
/// #[derive(Default, bk_sqlx::FromRow)]
/// #[bk_sqlx(default)]
/// struct Options {
///     option_a: Option<i32>,
...
/// `bk_sqlx::decode::Decode`, `bk_sqlx::type::Type`), in particular in case of nested structures.
/// For example:
///
/// ```rust,ignore
/// #[derive(bk_sqlx::FromRow)]
...
///
/// ```rust,ignore
/// let user: User = bk_sqlx::query_as("SELECT name FROM users")
...
///
/// ```rust,ignore
/// use bk_sqlx::{FromRow, sqlite::SqliteRow, bk_sqlx::Row};
/// struct MyCustomType {
///     custom: String,
/// }
///
...
///     fn from_row(row: &SqliteRow) -> bk_sqlx::Result<Self> {
///         Ok(Self {
///             bar: MyCustomType {
...
/// #### `json`
///
/// If your database supports a JSON type, you can leverage `#[bk_sqlx(json)]`
