    ...
    A: 'q + Send + IntoArguments<'q, DB>,
{
    ...
    #[deprecated = "Only the SQLite driver supports multiple statements in one prepared statement and that behavior is deprecated. Use `bk_sqlx::raw_sql()` instead."]
    pub fn fetch_many<'e, 'c: 'e, E>(
        mut self,
        executor: E,
...
/// let results = bk_sqlx::query(&query).fetch_all(&mut conn).await?;
/// # }
/// ```
///
...
///
/// See [the `types` module][crate::types] (links to `bk_sqlx_core::types` but you should use `bk_sqlx::types`) for details.
///
