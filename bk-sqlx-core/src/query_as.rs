...
///
/// See [`bk_sqlx::types`][crate::types] for the types that can be used.
///
...
///
/// ```rust,no_run
/// # async fn example1() -> bk_sqlx::Result<()> {
/// use bk_sqlx::Connection;
/// use bk_sqlx::PgConnection;
///
...
///     .await?;
///
/// bk_sqlx::query("INSERT INTO users(id, username) VALUES (1, 'alice'), (2, 'bob');")
///     .execute(&mut conn)
///     .await?;
///
/// // Get the first row of the result (note the `LIMIT 1` for efficiency)
/// // This assumes the `time` feature of SQLx is enabled.
/// let oldest_user: (i64, String, time::OffsetDateTime) = bk_sqlx::query_as(
...
///
/// // Get at most one row
/// let maybe_charlie: Option<(i64, String, time::OffsetDateTime)> = bk_sqlx::query_as(
...
/// let users: Vec<(i64, String, time::OffsetDateTime)> = bk_sqlx::query_as(
...
/// ```rust,no_run
/// # async fn example2() -> bk_sqlx::Result<()> {
/// use bk_sqlx::Connection;
/// use bk_sqlx::PgConnection;
///
...
///
/// // Get the first row of the result (note the `LIMIT 1` for efficiency)
/// let oldest_user: User = bk_sqlx::query_as(
///     "SELECT id, username, created_at FROM users ORDER BY created_at LIMIT 1"
/// )
///     .fetch_one(&mut conn)
///     .await?;
...
///
/// // Get at most one row
/// let maybe_charlie: Option<User> = bk_sqlx::query_as(
///     "SELECT id, username, created_at FROM users WHERE username = 'charlie'"
/// )
///     .fetch_optional(&mut conn)
///     .await?;
...
///
/// // Get all rows in result (Beware of the size of the result set! Consider using `LIMIT`)
/// let users: Vec<User> = bk_sqlx::query_as(
///     "SELECT id, username, created_at FROM users ORDER BY id"
/// )
///     .fetch_all(&mut conn)
///     .await?;
