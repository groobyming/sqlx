...
impl<'q> QueryLogger<'q> {
    ...
    pub fn finish(&self) {
        ...
        if let Some((tracing_level, log_level)) = private_level_filter_to_levels(lvl) {
            ...
            let log_is_enabled = log::log_enabled!(target: "bk_sqlx::query", log_level)
                || private_tracing_dynamic_enabled!(target: "bk_sqlx::query", tracing_level);
            if log_is_enabled {
