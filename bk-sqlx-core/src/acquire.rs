...
///
/// ```rust
/// # use bk_sqlx::{Acquire, postgres::Postgres, error::BoxDynError};
...
///
///     bk_sqlx::query!("SELECT 1 as v").fetch_one(&mut *conn).await?;
///     bk_sqlx::query!("SELECT 2 as v").fetch_one(&mut *conn).await?;
///
///     Ok(())
/// }
/// ```
///
/// If you run into a lifetime error about "implementation of `bk_sqlx::Acquire` is
...
///
/// ```rust
/// # use bk_sqlx::{postgres::PgConnection, error::BoxDynError};
/// # #[cfg(any(postgres_9_6, postgres_15))]
...
///
/// [workaround]: https://github.com/launchbadge/bk_sqlx/issues/1015#issuecomment-767787777
pub trait Acquire<'c> {
