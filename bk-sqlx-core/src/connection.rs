...
pub trait Connection: Send {
    ...
    ///
    /// ```rust
    /// use bk_sqlx::postgres::{PgConnection, PgRow};
    /// use bk_sqlx::Connection;
    ///
    /// # pub async fn _f(conn: &mut PgConnection) -> bk_sqlx::Result<Vec<PgRow>> {
    /// conn.transaction(|txn| Box::pin(async move {
    ///     bk_sqlx::query("select * from ..").fetch_all(&mut **txn).await
    /// })).await
    /// # }
    /// ```
