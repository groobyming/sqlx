...
///
/// You can create a new empty migration script using bk_sqlx-cli:
/// `bk_sqlx migrate add <DESCRIPTION>`.
///
/// Note that migrations for each database are tracked using the
/// `_bk_sqlx_migrations` table (stored in the database). If a migration's hash
