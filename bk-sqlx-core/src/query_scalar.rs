    ...
    (O,): Send + Unpin + for<'r> FromRow<'r, DB::Row>,
{
    ...
    #[inline]
    #[deprecated = "Only the SQLite driver supports multiple statements in one prepared statement and that behavior is deprecated. Use `bk_sqlx::raw_sql()` instead. See https://github.com/launchbadge/bk_sqlx/issues/3108 for discussion."]
...
/// # let mut conn: bk_sqlx::PgConnection = unimplemented!();
/// use uuid::Uuid;
///
/// // MySQL and MariaDB: use `?`
/// let user_id: Option<Uuid> = bk_sqlx::query_scalar("SELECT user_id FROM users WHERE username = $1")
///     .bind("alice")
...
///
/// ```rust,no_run
/// # async fn example_count() -> bk_sqlx::Result<()> {
/// # let mut conn: bk_sqlx::PgConnection = unimplemented!();
...
/// let count: i64 = bk_sqlx::query_scalar("SELECT COUNT(*) FROM users WHERE accepted_tos IS TRUE")
///     // Use `&mut` where `conn` is a connection or a transaction, or use `&` for a `Pool`.
///     .fetch_one(&mut conn)
///     .await?;
///
...
/// let (count,): (i64,) = bk_sqlx::query_as("SELECT COUNT(*) FROM users WHERE accepted_tos IS TRUE")
///     .fetch_one(&mut conn)
///     .await?;
/// # Ok(())
/// # }
...
///
/// ```rust,no_run
/// # async fn example_exists() -> bk_sqlx::Result<()> {
/// # let mut conn: bk_sqlx::PgConnection = unimplemented!();
/// // MySQL and MariaDB: use `?`
/// let username_taken: bool = bk_sqlx::query_scalar(
...
///
/// ```rust,no_run
/// # async fn example_aggregate() -> bk_sqlx::Result<()> {
/// # let mut conn: bk_sqlx::PgConnection = unimplemented!();
/// let max_upvotes: Option<i64> = bk_sqlx::query_scalar("SELECT MAX(upvotes) FROM posts")
///     // Use `&mut` where `conn` is a connection or a transaction, or use `&` for a `Pool`.
///     .fetch_one(&mut conn)
///     .await?;
/// # Ok(())
...
///
/// ```rust,no_run
/// # async fn example_vec() -> bk_sqlx::Result<()> {
/// # let mut conn: bk_sqlx::PgConnection = unimplemented!();
/// let top_users: Vec<String> = bk_sqlx::query_scalar(
