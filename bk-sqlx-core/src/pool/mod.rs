...
//!
//! ```rust,ignore
//! bk_sqlx::query("DELETE FROM articles").execute(&pool).await?;
//! ```
//!
...
///
/// If you're writing tests, consider using `#[bk_sqlx::test]` which handles the lifetime of
/// the pool for you.
///
...
impl<DB: Database> Pool<DB> {
    ...
    /// # async fn bleh() -> bk_sqlx_core::error::Result<()> {
    /// use bk_sqlx::PgPool;
    ///
