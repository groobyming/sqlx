...
///
/// ```rust,ignore
/// let user_ids: Vec<UserId> = bk_sqlx::query_scalar("select '{ 123, 456 }'::int8[]")
...
///
/// ```rust,ignore
/// #[derive(bk_sqlx::Type)] // ERROR: `Vec<i64>` does not implement `PgHasArrayType`
/// #[bk_sqlx(transparent)]
/// struct UserIds(Vec<i64>);
/// ```
///
/// To remedy this, add `#[bk_sqlx(no_pg_array)]`, which disables the generation
...
/// ##### Attributes
///
/// * `#[bk_sqlx(type_name = "<SQL type name>")]` on struct definition: instead of inferring the SQL
...
/// * `#[bk_sqlx(rename_all = "<strategy>")]` on struct definition: See [`derive docs in FromRow`](crate::from_row::FromRow#rename_all)
/// * `#[bk_sqlx(no_pg_array)]`: do not emit a `PgHasArrayType` impl (see above).
///
/// ### Enumeration
///
...
/// ```rust,ignore
/// #[derive(bk_sqlx::Type)]
/// #[bk_sqlx(type_name = "color")] // only for PostgreSQL to match a type definition
...
/// ```rust,ignore
/// #[derive(bk_sqlx::Type)]
/// #[bk_sqlx(type_name = "interface_type")]
