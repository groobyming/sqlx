...
// `PgAdvisoryLockGuard`.
//
// See: https://github.com/launchbadge/bk_sqlx/issues/2520
...
pub fn begin_ansi_transaction_sql(depth: usize) -> Cow<'static, str> {
    if depth == 0 {
        ...
        Cow::Owned(format!("SAVEPOINT _bk_sqlx_savepoint_{depth}"))
    }
}
...
pub fn commit_ansi_transaction_sql(depth: usize) -> Cow<'static, str> {
    if depth == 1 {
        ...
        Cow::Owned(format!("RELEASE SAVEPOINT _bk_sqlx_savepoint_{}", depth - 1))
    }
}
...
pub fn rollback_ansi_transaction_sql(depth: usize) -> Cow<'static, str> {
    if depth == 1 {
        ...
        Cow::Owned(format!(
            "ROLLBACK TO SAVEPOINT _bk_sqlx_savepoint_{}",
            depth - 1
        ))
    }
}
